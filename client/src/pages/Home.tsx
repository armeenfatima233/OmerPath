...
function StatCard({ label, value, detail, icon: Icon, accent = "saffron" }: { label: string; value: string | number; detail: string; icon: React.ComponentType<{size?:number}>; accent?: "saffron" | "sage" | "clay" }) {
  ...
  return <div className="paper-card motion-card p-5"><div className={`flex size-9 items-center justify-center rounded-xl ${tone}`}><Icon size={17}/></div><div className="mt-6 font-display text-[32px] tracking-[-.04em]">{value}</div><div className="mt-1 text-xs font-semibold text-ink/55">{label}</div><div className="mt-3 text-[11px] text-ink/40">{detail}</div></div>;
}
...
function Dashboard({ scholarships }: { scholarships: Scholarship[] }) {
  ...
  return <>
    <PageTitle eyebrow="Overview" title={<>Your next<br/><em>strongest move.</em></>} description="A clear view of the scholarships, documents, and deadlines that matter most to your path." action={<Link href="/dashboard/profile" className="button-primary"><Pencil size={15}/> Complete profile</Link>}/>
    <div className="route-strip mb-8"><div className="route-stop"><span className="route-dot route-dot-live"/><span>Profile</span></div><div className="route-track"/><div className="route-stop"><span className="route-dot"/><span>Matches</span><b>{scholarships.length} ranked</b></div><div className="route-track"/><div className="route-stop"><span className="route-dot"/><span>Applications</span><b>{applications.length} active</b></div></div>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Strong matches" value={scholarships.filter((s) => (s.match ?? 0) >= 79).length} detail="79% match or higher" icon={Target}/><StatCard label="Saved" value={saved.length} detail="On your shortlist" icon={BookmarkCheck} accent="sage"/><StatCard label="Applications" value={applications.length} detail="Across preparation stages" icon={ClipboardCheck} accent="clay"/><StatCard label="Passport readiness" value={`${readyDocs}/${PASSPORT_READINESS_TARGET}`} detail="Documents uploaded" icon={FileCheck2} accent="sage"/></div>
    <div className="mt-8 grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <div className="paper-card overflow-hidden"><div className="relative min-h-[260px]"><img src={ASSETS.students} alt="International students on a university campus" loading="lazy" className="absolute inset-0 size-full object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(20,33,38,.92),rgba(20,33,38,.22))]"/><div className="relative max-w-[560px] p-7 text-paper"><div className="eyebrow text-paper/55">Passport</div><h2 className="mt-4 font-display text-3xl leading-tight tracking-[-.04em]">Keep your documents ready.</h2><p className="mt-3 max-w-md text-sm leading-relaxed text-paper/65">Complete documents strengthen every application you submit.</p><Link href="/dashboard/passport" className="mt-6 button-primary bg-saffron">Open Passport <ArrowRight size={15}/></Link></div></div></div>
      <div className="paper-card p-6"><div className="flex items-center justify-between"><div><div className="eyebrow">Advisor brief</div><h2 className="mt-2 font-display text-2xl">What deserves attention now.</h2></div><Sparkles size={20} className="text-saffron"/></div><div className="mt-5 space-y-2"><ActionItem icon={FileText} title="Keep your Passport current" detail="Strengthens every application" href="/dashboard/passport"/>{next && next.match !== null && <ActionItem icon={GraduationCap} title={`${next.match}% match · ${next.name}`} detail="Review why the fit is strong" href={`/dashboard/scholarships/${next.id}`}/>}</div><Link href="/dashboard/advisor" className="mt-5 text-link">Ask the Advisor <ArrowRight size={14}/></Link></div>
    </div>
...
function Applications({ scholarships }: { scholarships: Scholarship[] }) {
  ...
  return <>
    ...
    <div className="application-image-strip rounded-[24px]"><img src={ASSETS.application} alt="Student reviewing an application" loading="lazy"/><div><div className="eyebrow text-paper/55">Application pulse</div><h2 className="mt-2 font-display text-2xl text-paper">Every application, tracked in one place.</h2></div></div>
...
function Passport() {
  ...
  const identity = getUserIdentity(user);
  const shareSlug = identity.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "student";
  const shareLink = `omerpath.app/p/${shareSlug}`;
  ...
  return <>
    ...
    {shareOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="share-title"><div className="modal-card"><div className="flex items-start justify-between"><div><div className="eyebrow">Passport sharing</div><h2 id="share-title" className="mt-2 font-display text-2xl">Control what leaves your workspace.</h2></div><button aria-label="Close share dialog" className="icon-button" onClick={() => setShareOpen(false)}><X size={17}/></button></div><p className="mt-3 text-sm leading-relaxed text-ink/55">Anyone with this link can view your shared documents until it expires.</p><label className="settings-row mt-5"><span><b>Link expiry</b><small>How long the link stays active.</small></span><select value={expiry} onChange={(e) => setExpiry(e.target.value)}><option>24 hours</option><option>7 days</option><option>30 days</option></select></label><label className="settings-row"><span><b>Allow document downloads</b><small>Off keeps sharing read-only.</small></span><input type="checkbox" checked={allowDownload} onChange={(e) => setAllowDownload(e.target.checked)}/></label><div className="share-link"><Link2 size={15}/><span>{shareLink}</span><button aria-label="Copy share link" onClick={() => { navigator.clipboard?.writeText(`https://${shareLink}`); toast(`Link copied · expires in ${expiry}`); }}><Copy size={15}/></button></div><div className="mt-5 flex justify-end gap-2"><button className="button-secondary" onClick={() => setShareOpen(false)}>Cancel</button><button className="button-primary" onClick={() => { toast(`Share link created · ${expiry} · downloads ${allowDownload ? "allowed" : "blocked"}`); setShareOpen(false); }}>Create share link</button></div></div></div>}
  </>;
}
...
function Advisor() {
  ...
  const prompts = ["Why am I a good fit for my top match?", "What documents am I missing?", "Compare my top 3"];
  return <>
